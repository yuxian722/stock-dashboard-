...
基本資訊區勾選「疊層(一次上兩顆)」後，會多出主層/次層f9欄位輸入框。選座標區改成**同時顯示兩張
BINGO MAP**(主層/次層各一張，各自獨立的格子圖、hover座標顯示、已選清單)，不用再切換畫面比對。
產生時會呼叫`assign_two_layers()`，輸出檔案會有`[DIE_INFO_BEG]`(主層)跟`[DIE_INFO_OTHER_LAYER_BEG]`
(次層)兩段。

**共用同一片wafer時**(預設)：wafer圖只有一張，上方有主層/次層切換按鈕決定目前點wafer座標是加到
哪一層；已經被任一層點過的座標**全域**不能再被另一層重複點選(藍框=主層已選、紫框=次層已選，可能
同時出現在同一張wafer圖上)——這是為了解決「切換主層/次層畫面時很容易點錯、選到重複座標」的問題。

**跨兩片wafer時**：再勾選「主層/次層來自不同的wafer」後，會多出第二個wafer讀取區塊(FRM或貼文字都
跟主層一樣獨立一份)，畫面同時出現**兩張wafer圖**可以直接點——左圖點選一律加入主層、右圖點選一律
加入次層，不需要切換按鈕；因為是兩片實體wafer，兩張圖之間的座標即使數值相同也不算重複，各自獨立
判斷是否已選過。
