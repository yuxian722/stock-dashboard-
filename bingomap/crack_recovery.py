...
`local_view()` is a plain identity normalization (no flip, no rotation) —
confirmed by extension from the same 2026/08/17 real DB evidence that
fixed mispick_analysis.py's coordinate transform: since that evidence
showed STRATE `wafer_xy` already equals the wafer MAP's own raw coordinate
for DB with no flip anywhere, there is no reason for this module's
*display* scatter to introduce a flip DB's own tooling doesn't have. This
specific consequence (the scatter's own orientation) was not independently
re-checked against a DB Crack scenario — it is inferred by consistency,
and is a display-only choice, not a bin/pass-fail decision.

2026/09/03: this module used to also take a `machine_type` ("DB"/"ESEC")
on every function, mirroring mispick_analysis.py's since-removed ESEC
support — removed for the same reason, see that module's docstring.
"""
...
def local_view(fx: int, fy: int, rng: WaferPoolRange) -> tuple[int, int]:
    ...
    docstring). Plain identity normalization (no flip, no rotation) — see
    module docstring for the evidence basis."""
...
def crack_csv_rows(session: CrackSession, marked_keys: list[str]) -> list[list]:
    ...
    for i, c in enumerate(session.marked(marked_keys), start=1):
        ...
        rng = wafer_pool_range(pool)
        notch = wafer_notch(pool)
        view_x, view_y = local_view(c.fx, c.fy, rng)
        rows.append(
            [
                f"C{i}",
                c.substrate_id,
